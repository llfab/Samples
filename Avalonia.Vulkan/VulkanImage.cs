...
namespace Avalonia.Vulkan
{
    public class VulkanImage : IDisposable
    {
        ...
        public VulkanImage(VulkanDevice device, VulkanPhysicalDevice physicalDevice, VulkanCommandBufferPool commandBufferPool, uint format, PixelSize size, uint mipLevels = 0, bool externalFlags = false)
        {
            _device = device;
            ...
            Initialize(externalFlags);
        }

        public unsafe void Initialize(bool externalFlag = false)
        {
            if (!InternalHandle.HasValue)
            {
                ...
                if (externalFlag)
                {
                    ...
                    imageCreateInfo = new ImageCreateInfo
                    {
                ...
                if (externalFlag)
                {
                    MemoryGetWin32HandleInfoKHR memoryFdInfo = new MemoryGetWin32HandleInfoKHR()
                    {
                    ...
                    _device.Api.GetBufferMemoryRequirements();
                }
                */
