    ...
    public abstract class VulkanControlBase : Control
    {
        ...
        private class VulkanDrawOperation : ICustomDrawOperation
        {
            ...
            public void Render(IDrawingContextImpl context)
            {
                if (_bitmap == null)
                    return;
                //if (context is CompositorDrawingContextProxy)
                //{
                //    context = (context as CompositorDrawingContextProxy).InternalContext;
                //}
                ...
                using (_bitmap.Lock())
                {
                        ...
                        SKColorType.Bgra8888, SKColorSpace.CreateSrgb()))
                    {
                        ...
                        {
                            var leaseStuff = leaseFeature.Lease();
                            leaseStuff.SkCanvas.DrawImage(snapshot, _srcRect.ToSKRect(), _dstRect.ToSKRect(), new SKPaint());
                            leaseStuff.Dispose();
